...
1. **本地代码与文档**：`search_code`/`sym_query`/`get_source` 查 tools/sources.json
   ...
   必须看到原文才算数，证据留 文件:行号。
2. **互联网权威源**：`WebSearch` / `WebFetch` —— 官方文档 > 官方仓库源码 >
   论文/规范 > 一手技术博客 > 社区讨论。引用必须带 **URL + 访问日期**，
   关键结论摘引原文；版本/时效敏感的信息注明版本号与日期。
...
- **B · 外部调研**：WebSearch 撒网 → WebFetch 精读权威源 → **≥2 个独立来源
  交叉验证**才下结论；单源结论必须标注"单源未验证"。
